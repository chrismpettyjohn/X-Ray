...
export function CustomizeProfileForm({ defaultUser, onChanges }: CustomizeProfileFormProps) {
      ...
      <Paper elevation={3} sx={{background: '#24242D', p: 2, mb: 4}}>
        <Grid container spacing={2} sx={{mb: 4}}>
          <Grid item xs={12}>
            ...
            <Typography variant="caption">Your profile is how others see you</Typography>
          </Grid>
          <Grid item xs={12}>
            <ProfilePictureEditor defaultProfilePicture={defaultUser?.profilePicture} onChangeProfilePicture={onChangeProfilePicture} />
          </Grid>
        </Grid>
      </Paper>
      <Paper elevation={3} sx={{background: '#24242D', p: 2, mb: 4}}>
        <Grid container sx={{mb: 4}}>
          <Grid item xs={12}>
            ...
            <Typography variant="caption">Other users can only see your communication preferences if it's enabled.</Typography>
          </Grid>
        </Grid>
        <Grid container sx={{mb: 2}}>
          ...
          <Grid item xs={12}>
            <TextField id="username" name="username" variant="standard" value={privateUserDTO?.username} onChange={updatePrivateUserDTO} fullWidth sx={{mt: 2}} />
          </Grid>
        </Grid>
      </Paper>
      <Paper elevation={3} sx={{background: '#24242D', p: 2, mb: 4}}>
        <Grid container spacing={2} sx={{mb: 4}}>
          <Grid item xs={12}>
            ...
            <Typography variant="caption">Make sure to store your new password somewhere safe so you don't forget</Typography>
          </Grid>
          <Grid item xs={6}>
            <PasswordField label="Existing Password" password={privateUserDTO.existingPassword} onChange={value => updatePrivateUserDTORaw({ existingPassword: value })} />
          </Grid>
          <Grid item xs={6}>
            <PasswordField label="New Password" password={privateUserDTO.newPassword} onChange={value => updatePrivateUserDTORaw({ newPassword: value })} />
          </Grid>
        </Grid>
      </Paper>
      <Grid container sx={{mb: 2}}>
