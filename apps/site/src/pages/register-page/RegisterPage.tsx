...
import {GOOGLE_RECAPTCHA_CLIENT_KEY, GuestGuard, sessionContext, userService} from '@xray/web';
        ...
        >
          <Card sx={{padding: 2, width: '100%'}}>
            <form onSubmit={onCreateAccount}>
              ...
              <TextField id="username" name="username" label="Username" variant="outlined" value={username} onChange={event => setUsername(event.target.value)} fullWidth sx={{mt: 2}} />
              <PasswordField id="password" name="password" variant="outlined" fullWidth value={password} onChange={setPassword} sx={{mt: 2}} required />
              <PasswordField id="password_again" name="password_again" label="Password Again" variant="outlined" fullWidth value={passwordAgain} onChange={setPasswordAgain} sx={{mt: 2}} required />
              <Box sx={{mt: 2}}>
                <ReCAPTCHA
